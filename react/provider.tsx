...
export function useStoreState<TState extends object, TSlice extends object>(
    projection: (state: TState) => TSlice,
): TSlice;
export function useStoreState<TState extends object, TSlice extends object = TState>(
    projection?: (state: TState) => TSlice,
): TSlice {
    const store = useStore<TState>();
    const [slice, setSlice] = React.useState<TSlice>(
        projection ? projection(store.currentState) : ((store.currentState as unknown) as TSlice),
    );
 ...
 */
export function useStoreSlices<TState extends object>(): <TSlice extends object>(
    projection: (state: TState) => TSlice,
) => TSlice {
