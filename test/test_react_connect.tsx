...
describe("react bridge: connect() tests", () => {
    ...
    beforeEach(() => {
        ...
        store.destroyed.subscribe(() => cleanup.unsubscribe());
        mountInsideStoreProvider = (elem: JSX.Element) =>
            Enzyme.mount(<StoreProvider store={store}>{elem}</StoreProvider>);
    });
        ...
        const Root: React.SFC<{ message?: string }> = (props) => {
            return (
                <StoreProvider store={store}>
                    <ConnectedTestComponent message={props.message} />
                </StoreProvider>
            );
        };
    ...
    it("should be possible to add additional props to a connected component and subscribe to it in the connect callback", (done) => {
        ...
        const ConnectedTestComponent = connect(
            Component,
            (store: Store<TestState>, inputProps: Observable<InputProps>) => {
                inputProps.pipe(take(1)).subscribe((props) => {
                    expect(props).to.deep.equal({ test: 1 });
                });

                inputProps.pipe(skip(1), take(1)).subscribe((props) => {
        ...
        const Root: React.SFC<any> = (props: any) => (
            <StoreProvider store={store}>
    ...
    it("should be possible to infer the inputProps type", (done) => {
        ...
        const ConnectedTestComponent = connect<TestState, ComponentProps, InputProps>(
            Component,
            (store, inputProps) => {
